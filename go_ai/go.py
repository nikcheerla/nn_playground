...
def softmax(weights, temp):
    prob_t = [0]*len(weights)
    denominator = 0
    for a in range(0, len(weights)):
        denominator += np.exp(weights[a]/temp)  #calculate numerators

    for a in range(0, len(weights)):
        prob_t[a] = np.exp(weights[a]/temp)/denominator

    return weighted_choice(prob_t)
...
def getNNet():
    global newest
    global net
    newfile = max(glob.iglob('*.pkl'), key=os.path.getctime)
    if newest is None or newfile != newest:
        newest = newfile
...
def selectmove(xoro):
    ...
    for i in range(0, BOARD_SIZE):
        for j in range(0, BOARD_SIZE):
            if int(board_arr[i][j]) == 0:
                board_arr[i][j] = -1
                prob = 1 - float(getNNet().predict(board_arr.reshape((-1, 1, 28, 28)))[0])/100.0
                poss.append([i, j])
                probs.append(prob)
                board_arr[i][j] = 0
    print probs
    choice = softmax(probs, aitemp)
    print choice
    maxi = poss[choice][0]
    maxj = poss[choice][1]
    ...
    board_at_turn[turncount] = np.zeros((28, 28))
    board_at_turn[turncount] = board_at_turn[turncount].astype(int)
    for i in range(0, BOARD_SIZE):
        for j in range(0, BOARD_SIZE):
            if gsf[i][j] == 'o':
                board_at_turn[turncount][i][j] = 1
            elif gsf[i][j] == 'x':
                board_at_turn[turncount][i][j] = 2
...
def main():
    ...
    for tt in range(1, turncount):
        st = `tt%2 + 1` + " " + `winner` + "\n"
        for i in range(0, BOARD_SIZE):
            for j in range(0, BOARD_SIZE):
                st += `board_at_turn[tt][i][j]`
            st += "\n"

        fname = "data/" + `BOARD_SIZE` + "x" + `BOARD_SIZE` + "/board" + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + `random.randrange(1, 1000000)` + ".txt"
        print fname
